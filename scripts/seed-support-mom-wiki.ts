/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 서폿 Mom 위키·공통 상대법 시드.
 * 세라핀·카르마는 비원딜 배치의 공유 문서를 재사용하며 나머지 빈 문서만 채운다.
 * 실행: npx tsx scripts/seed-support-mom-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/support-mom-wiki.sql
 */
...
const CREATED_AT = "2026-09-15T10:10:00.000Z";
const PATCH = catalog.patch;
const slugs = ["lulu", "soraka", "nami", "janna", "yuumi", "milio", "sona", "taric"] as const;
...
for (const slug of slugs) {
  const champion = championFor(slug);
  const body = readManuscript("seeds/support-mom-matchup-wiki", slug);
  validateMatchup(slug, body);
  appendNewMatchup(slug, body);
  console.log(`${champion.name}: 상대법 ${body.length}자, 검증 통과`);
}

writeFileSync("seeds/support-mom-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(`생성: seeds/support-mom-wiki.sql (챔피언 위키 ${slugs.length}개 + 상대법 ${slugs.length}개)`);

function championFor(slug: (typeof slugs)[number]) {
  ...
  assert(
    classifications.some(
      (entry) =>
        ...
        entry.categorySlug === "mom" &&
        entry.championName === champion.name,
    ),
    `서폿 Mom 초기 분류에 없는 챔피언: ${champion.name}`,
  );
  return champion;
}
...
function validateArticle(slug: string, body: string) {
  ...
  assert(body.includes("[[분류:서폿]] [[분류:Mom]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-support-mom-champion-${slug}-20260915`;
  const editId = `edit-support-mom-champion-${slug}-20260915`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '서폿 Mom 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-support-mom-matchup-${slug}-20260915`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '서폿 Mom 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
