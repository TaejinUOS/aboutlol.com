/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 정글 AD 위키·공통 상대법 시드.
 * 올라프 일반 위키·상대법과 리 신 상대법은 기존 공유 문서와 편집 이력을 보존한다.
 * 실행: npx tsx scripts/seed-jungle-ad-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/jungle-ad-wiki.sql
 */
...
const CREATED_AT = "2026-09-15T00:30:00.000Z";
...
for (const slug of matchupSlugs) {
  const champion = championFor(slug);
  const body = readManuscript("seeds/jungle-ad-matchup-wiki", slug);
  validateMatchup(slug, body);
  appendNewMatchup(slug, body);
  console.log(`${champion.name}: 상대법 ${body.length}자, 검증 통과`);
}

writeFileSync("seeds/jungle-ad-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(
  `생성: seeds/jungle-ad-wiki.sql (챔피언 위키 ${articleSlugs.length}개 + 상대법 ${matchupSlugs.length}개)`,
);
...
function championFor(slug: (typeof articleSlugs)[number] | (typeof matchupSlugs)[number]) {
  ...
  assert(
    ...
    `정글 AD 초기 분류에 없는 챔피언: ${champion.name}`,
  );
  return champion;
}
...
function validateArticle(slug: string, body: string) {
  ...
  assert(body.includes("[[분류:정글]] [[분류:AD]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-jungle-ad-champion-${slug}-20260915`;
  const editId = `edit-jungle-ad-champion-${slug}-20260915`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '정글 AD 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-jungle-ad-matchup-${slug}-20260915`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '정글 AD 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
