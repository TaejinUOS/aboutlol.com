/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 서폿 Dad 위키·공통 상대법 시드.
 * 탐 켄치·뽀삐 일반 위키와 블리츠크랭크·노틸러스·탐 켄치·뽀삐 상대법은 보존한다.
 * 실행: npx tsx scripts/seed-support-dad-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/support-dad-wiki.sql
 */
...
const CREATED_AT = "2026-09-15T10:35:00.000Z";
const PATCH = catalog.patch;
const articleSlugs = [
  "blitzcrank", "thresh", "leona", "alistar", "nautilus", "rakan", "braum", "rell", "pyke",
  "renata",
] as const;
const matchupSlugs = [
  "thresh", "leona", "alistar", "rakan", "braum", "rell", "pyke", "renata",
] as const;
...
for (const slug of matchupSlugs) {
  const champion = championFor(slug);
  const body = readManuscript("seeds/support-dad-matchup-wiki", slug);
  validateMatchup(slug, body);
  appendNewMatchup(slug, body);
  console.log(`${champion.name}: 상대법 ${body.length}자, 검증 통과`);
}

writeFileSync("seeds/support-dad-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(
  `생성: seeds/support-dad-wiki.sql (챔피언 위키 ${articleSlugs.length}개 + 상대법 ${matchupSlugs.length}개)`,
);
...
function championFor(slug: (typeof articleSlugs)[number] | (typeof matchupSlugs)[number]) {
  ...
  assert(
    classifications.some(
      (entry) =>
        entry.positionSlug === "support" &&
        entry.categorySlug === "dad" &&
        entry.championName === champion.name,
    ),
    `서폿 Dad 초기 분류에 없는 챔피언: ${champion.name}`,
  );
  return champion;
}
...
function validateArticle(slug: string, body: string) {
  assert(body.startsWith("# 서폿 실전 운용\n\n> **한눈에 보기** — "));
  assert(body.length <= MAX_BODY_LENGTH, `${slug}: 챔피언 위키 길이 초과`);
  assert(body.includes("[[분류:서폿]] [[분류:Dad]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-support-dad-champion-${slug}-20260915`;
  const editId = `edit-support-dad-champion-${slug}-20260915`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '서폿 Dad 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-support-dad-matchup-${slug}-20260915`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '서폿 Dad 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
