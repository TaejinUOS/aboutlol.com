/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 탑 딜탱 나머지 12명의
 * 챔피언 위키·공통 상대법 시드. 기존 문서나 편집 이력이 있으면 덮어쓰지 않는다.
 * 실행: npx tsx scripts/seed-top-bruiser-remaining-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/top-bruiser-remaining-wiki.sql
 */
...
const CREATED_AT = "2026-09-14T14:30:00.000Z";
...
writeFileSync("seeds/top-bruiser-remaining-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(
  `생성: seeds/top-bruiser-remaining-wiki.sql (챔피언 위키 ${slugs.length}개 + 상대법 ${slugs.length}개)`,
);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-top-bruiser-remaining-champion-${slug}-20260914`;
  const editId = `edit-top-bruiser-remaining-champion-${slug}-20260914`;
  lines.push(
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-top-bruiser-remaining-matchup-${slug}-20260914`;
  lines.push(
