/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 비원딜 위키·공통 상대법 시드.
 * 이미 작성된 공유 문서와 편집 이력은 보존하고 빈 문서만 채운다.
 * 실행: npx tsx scripts/seed-non-adc-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/non-adc-wiki.sql
 */
...
const CREATED_AT = "2026-09-15T09:50:00.000Z";
const PATCH = catalog.patch;
const articleSlugs = [
  "ziggs", "seraphine", "swain", "velkoz", "karma", "pantheon", "zyra", "brand",
] as const;
const matchupSlugs = [
  "ziggs", "seraphine", "swain", "velkoz", "karma", "hwei", "pantheon", "zyra", "brand",
] as const;
...
for (const slug of matchupSlugs) {
  const champion = championFor(slug);
  const body = readManuscript("seeds/non-adc-matchup-wiki", slug);
  validateMatchup(slug, body);
  appendNewMatchup(slug, body);
  console.log(`${champion.name}: 상대법 ${body.length}자, 검증 통과`);
}

writeFileSync("seeds/non-adc-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(
  `생성: seeds/non-adc-wiki.sql (챔피언 위키 ${articleSlugs.length}개 + 상대법 ${matchupSlugs.length}개)`,
);
  ...
  assert(
    classifications.some(
      (entry) =>
        ...
        entry.categorySlug === "non-adc" &&
        entry.championName === champion.name,
    ),
    `비원딜 초기 분류에 없는 챔피언: ${champion.name}`,
  );
  return champion;
}
...
function validateArticle(slug: string, body: string) {
  ...
  assert(body.includes("[[분류:원딜]] [[분류:비원딜]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-non-adc-champion-${slug}-20260915`;
  const editId = `edit-non-adc-champion-${slug}-20260915`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '비원딜 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-non-adc-matchup-${slug}-20260915`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '비원딜 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
