/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 정글 AP 위키·공통 상대법 시드.
 * 다이애나·에코·자크·마오카이·럼블의 기존 공유 문서와 편집 이력은 보존한다.
 * 실행: npx tsx scripts/seed-jungle-ap-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/jungle-ap-wiki.sql
 */
...
const CREATED_AT = "2026-09-15T00:50:00.000Z";
...
const matchupSlugs = articleSlugs;
...
for (const slug of matchupSlugs) {
  const champion = championFor(slug);
  const body = readManuscript("seeds/jungle-ap-matchup-wiki", slug);
  validateMatchup(slug, body);
  appendNewMatchup(slug, body);
  console.log(`${champion.name}: 상대법 ${body.length}자, 검증 통과`);
}

writeFileSync("seeds/jungle-ap-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(
  `생성: seeds/jungle-ap-wiki.sql (챔피언 위키 ${articleSlugs.length}개 + 상대법 ${matchupSlugs.length}개)`,
);

function championFor(slug: (typeof articleSlugs)[number]) {
  ...
  assert(
    classifications.some(
      (entry) =>
        ...
        entry.categorySlug === "ap" &&
        entry.championName === champion.name,
    ),
    `정글 AP 초기 분류에 없는 챔피언: ${champion.name}`,
  );
  return champion;
}
...
function validateArticle(slug: string, body: string) {
  assert(body.startsWith("# 정글 실전 운용\n\n> **한눈에 보기** — "));
  assert(body.length <= MAX_BODY_LENGTH, `${slug}: 챔피언 위키 길이 초과`);
  assert(body.includes("[[분류:정글]] [[분류:AP]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-jungle-ap-champion-${slug}-20260915`;
  const editId = `edit-jungle-ap-champion-${slug}-20260915`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '정글 AP 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-jungle-ap-matchup-${slug}-20260915`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '정글 AP 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
