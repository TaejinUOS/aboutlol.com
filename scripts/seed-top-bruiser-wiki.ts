/**
 * 인벤 공략을 현재 스킬 구조와 대조해 작성한 탑 딜탱 첫 10명의 챔피언 위키·상대법 시드.
 * 새 문서는 비어 있을 때만 채우고, 기존 이렐리아 시스템 원고는 조사한 리비전과 이력이
 * 정확히 일치할 때만 탑 기준으로 보강한다.
 * 실행: npx tsx scripts/seed-top-bruiser-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/top-bruiser-wiki.sql
 */
...
for (const slug of slugs) {
  ...
  assert(
    ...
    `탑 딜탱 초기 분류에 없는 챔피언: ${champion.name}`,
  );

  const article = readFileSync(join("seeds/champion-wiki", `${slug}.md`), "utf8")
    .replace(/\r\n?/g, "\n")
    .trim();
  const matchup = readFileSync(join("seeds/top-bruiser-matchup-wiki", `${slug}.md`), "utf8")
    .replace(/\r\n?/g, "\n")
    .trim();
  ...
  if (slug === "irelia") {
    appendIreliaUpgrade(article, matchup);
  } else {
...
writeFileSync("seeds/top-bruiser-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(`생성: seeds/top-bruiser-wiki.sql (챔피언 위키 ${slugs.length}개 + 상대법 ${slugs.length}개)`);
...
function validateArticle(slug: string, body: string) {
  ...
  assert(body.includes("[[분류:탑]] [[분류:딜탱]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-top-bruiser-champion-${slug}-20260914`;
  const editId = `edit-top-bruiser-champion-${slug}-20260914`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '탑 딜탱 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-top-bruiser-matchup-${slug}-20260914`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '탑 딜탱 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
...
function appendIreliaUpgrade(article: string, matchup: string) {
  const articleId = "doc-ai-champion-irelia-20260912";
  const articleEdit = "edit-top-bruiser-champion-irelia-20260914";
  const matchupId = "doc-c-irelia";
  const matchupEdit = "edit-top-bruiser-matchup-irelia-20260914";
  lines.push(
    "-- 이렐리아 챔피언 위키: 인라인 출처 이관까지 끝난 시스템 원고만 보강",
    "UPDATE wiki_docs",
    `SET general = ${quote(article)}, revision = 3, patch = ${quote(PATCH)}, updated_at = ${quote(CREATED_AT)}, updated_by = ${quote(SYSTEM_USER_ID)}`,
    `WHERE id = ${quote(articleId)} AND kind = 'article' AND revision = 2 AND updated_by = ${quote(SYSTEM_USER_ID)}`,
    `AND (SELECT COUNT(*) FROM wiki_edits WHERE doc_id = ${quote(articleId)}) = 2`,
    `AND EXISTS (SELECT 1 FROM wiki_edits WHERE id = 'edit-ai-champion-irelia-20260912' AND doc_id = ${quote(articleId)} AND status = 'accepted')`,
    `AND EXISTS (SELECT 1 FROM wiki_edits WHERE id = 'edit-inline-sources-article-irelia-20260913' AND doc_id = ${quote(articleId)} AND status = 'accepted' AND revision = 2)`,
    `AND NOT EXISTS (SELECT 1 FROM wiki_edits WHERE id = ${quote(articleEdit)});`,
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(articleEdit)}, id, NULL, 2, general, '탑 딜탱 챔피언 위키 보강', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 3 FROM wiki_docs`,
    `WHERE id = ${quote(articleId)} AND revision = 3 AND general = ${quote(article)}`,
    `AND NOT EXISTS (SELECT 1 FROM wiki_edits WHERE id = ${quote(articleEdit)});`,
  ...
  lines.push(
    ...
    "UPDATE wiki_docs",
    `SET general = ${quote(matchup)}, revision = 3, patch = ${quote(PATCH)}, updated_at = ${quote(CREATED_AT)}, updated_by = ${quote(SYSTEM_USER_ID)}`,
    `WHERE id = ${quote(matchupId)} AND kind = 'matchup' AND revision = 2 AND updated_by = ${quote(SYSTEM_USER_ID)}`,
    `AND (SELECT COUNT(*) FROM wiki_edits WHERE doc_id = ${quote(matchupId)}) = 2`,
    `AND EXISTS (SELECT 1 FROM wiki_edits WHERE id = 'edit-ai-mid-matchup-irelia-20260912' AND doc_id = ${quote(matchupId)} AND status = 'accepted')`,
    `AND EXISTS (SELECT 1 FROM wiki_edits WHERE id = 'edit-inline-sources-matchup-irelia-20260913' AND doc_id = ${quote(matchupId)} AND status = 'accepted' AND revision = 2)`,
    `AND NOT EXISTS (SELECT 1 FROM wiki_edits WHERE id = ${quote(matchupEdit)});`,
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(matchupEdit)}, id, NULL, 2, general, '탑 딜탱 상대법 보강', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 3 FROM wiki_docs`,
    `WHERE id = ${quote(matchupId)} AND revision = 3 AND general = ${quote(matchup)}`,
    `AND NOT EXISTS (SELECT 1 FROM wiki_edits WHERE id = ${quote(matchupEdit)});`,
    "",
  );
}
