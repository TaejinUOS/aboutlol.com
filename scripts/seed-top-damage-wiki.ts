/**
 * 인벤 챔피언 정보와 현재 스킬 구조를 대조해 작성한 탑 딜러 위키·공통 상대법 시드.
 * 아크샨·리산드라 일반 위키와 리산드라 상대법은 기존 공유 문서를 보존한다.
 * 실행: npx tsx scripts/seed-top-damage-wiki.ts
 * 적용: npx wrangler d1 execute kkaenam-gg --local --file seeds/top-damage-wiki.sql
 */
...
const CREATED_AT = "2026-09-14T14:50:00.000Z";
...
for (const slug of matchupSlugs) {
  const champion = championFor(slug);
  const body = readManuscript("seeds/top-damage-matchup-wiki", slug);
  validateMatchup(slug, body);
  appendNewMatchup(slug, body);
  console.log(`${champion.name}: 상대법 ${body.length}자, 검증 통과`);
}

writeFileSync("seeds/top-damage-wiki.sql", `${lines.join("\n").trimEnd()}\n`);
console.log(
  `생성: seeds/top-damage-wiki.sql (챔피언 위키 ${articleSlugs.length}개 + 상대법 ${matchupSlugs.length}개)`,
);
...
function championFor(slug: (typeof articleSlugs)[number] | (typeof matchupSlugs)[number]) {
  ...
  assert(
    ...
    `탑 딜러 초기 분류에 없는 챔피언: ${champion.name}`,
  );
  return champion;
}
...
function validateArticle(slug: string, body: string) {
  ...
  assert(body.includes("[[분류:탑]] [[분류:딜러]]"), `${slug}: 분류 누락`);
...
function appendNewArticle(slug: string, name: string, body: string) {
  const docId = `doc-top-damage-champion-${slug}-20260914`;
  const editId = `edit-top-damage-champion-${slug}-20260914`;
  lines.push(
    ...
    `SELECT ${quote(editId)}, id, NULL, 0, general, '탑 딜러 챔피언 위키 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
...
function appendNewMatchup(slug: string, body: string) {
  ...
  const editId = `edit-top-damage-matchup-${slug}-20260914`;
  lines.push(
    ...
    "INSERT INTO wiki_edits (id, doc_id, me_slug, base_revision, body, summary, status, author, created_at, accepted_via, revision)",
    `SELECT ${quote(editId)}, id, NULL, 0, general, '탑 딜러 상대법 작성', 'accepted', ${quote(SYSTEM_USER_ID)}, ${quote(CREATED_AT)}, 'admin', 1 FROM wiki_docs`,
    `WHERE id = ${quote(docId)} AND revision = 1 AND general = ${quote(body)}`,
