...
**오른이 라인 뒤에서 제작을 시작하면 그 짧은 시간 동안 막타와 위치를 먼저 잡는다.** 제작을 끊겠다고 큰 웨이브를 가로질러 들어가면 미니언 피해로 교환을 진다. 대신 라인을 밀어 귀환 선택을 어렵게 만들거나, 강가 시야를 먼저 잡아 다음 웨이브의 주도권으로 바꾼다.

## `Q` 기둥이 생긴 뒤에는 옆으로 빠진다

**`용암 균열(Q)`에 맞았다면 뒤로만 달리지 않는다.** 내 뒤의 자연 지형과 새로 생길 기둥이 `화염 돌진(E)` 한 번에 이어지지 않는지 본다. **벽과 기둥 사이에 서 있는 순간이 오른의 가장 단순한 띄우기 각**이다. `E`가 빠진 뒤에는 오른의 즉시 접근 수단이 줄어드니 짧게 체력이나 라인 이득을 돌려받는다.
...
- [입문자를 위한 오른 파헤치기](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=137340)
- [오른 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=138)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
- [다이아) 캐리형 최대추댐 탑 탱커 말파이트](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=133569&vtype=pc)
- [말파이트 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=36)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**쉔 본체만 보면 `Q` 강화 공격 각을 놓친다.** 기의 검과 쉔 사이에 서 있으면 검이 통과하며 쉔의 다음 공격이 강해진다. 막타를 먹을 때 두 점을 한 줄로 만들지 않고 옆으로 선다. 검이 쉔 가까이에 이미 있다면 통과 강화가 어려워져 짧게 견제하기 좋다.

## `W` 안에서 기본 공격을 낭비하지 않는다

**기의 검 주변에 결계가 생기면 기본 공격을 잠시 멈추고 위치를 바꾼다.** 강화 평타나 공격 속도 증가 기술을 결계에 쏟지 않는다. `W`가 끝난 뒤에도 쉔의 `E`가 남아 있다면 바로 추격하지 말고 도발 경로를 옆으로 비튼다.
...
- [쉔은 절대 라인전 약캐가 아닙니다](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=115811&vtype=pc)
- [쉔 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=55)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**크산테가 `Q`를 두 번 맞힌 뒤에는 다음 타격이 끌어당김으로 바뀐다.** 준비 상태에서 정면으로 막타를 먹으면 `Q`부터 `W`까지 연계될 수 있다. 미니언 옆으로 각도를 바꾸거나 잠시 거리를 내주고, 세 번째 `Q`가 빗나간 뒤 짧게 압박한다.

## 표식이 묻은 상태에서 기본 공격 거리를 주지 않는다

**크산테의 기술에 맞으면 표식이 생기고 다음 기본 공격이 추가 피해를 만든다.** 기술 한 번을 맞았다고 즉시 맞딜로 들어가기보다 한 걸음 빠져 표식 소비를 어렵게 한다. `E`로 따라왔다면 보호막이 끝난 뒤 되받아치되, 아군 미니언을 타고 더 멀리 움직일 수 있다는 점을 계산한다.
...
- [크산테의 핵심 정동열](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=148170&vtype=pc)
- [크산테 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=162)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
- [사이온 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=57)', revision = 2, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**`대량 학살 강타(Q)`의 충전이 보이면 최대 사거리 밖으로 뛰기보다 직각에 가깝게 옆으로 벗어난다.** 수풀과 시야 밖에서는 시작 동작이 보이지 않으므로 사이온이 사라진 쪽 벽을 따라 걷지 않는다. 사이온이 짧게 끊었다면 다시 바로 붙지 말고 다음 기술과 미니언 수를 확인한다.

## 미니언 뒤가 안전지대가 아니다

**`학살자의 포효(E)`는 미니언을 뒤로 날려 그 경로의 챔피언에게 둔화와 방어력 감소를 건다.** 낮은 체력 미니언과 사이온을 한 줄에 두지 않는다. `E`가 빗나가면 사이온은 먼 거리에서 `Q`를 준비하기 어려워지므로 막타나 짧은 견제를 돌려받는다.
...
- [정석 착취 초가스 공략](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=146725)
- [초가스 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=11)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
- [뽀삐 공략 게시판](https://lol.inven.co.kr/dataninfo/champion/manualTool.php?champnum=49)
- [뽀삐 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=49)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**마오카이가 오래 점유한 수풀에는 강화된 묘목이 있을 가능성이 높다.** 체력으로 확인하지 말고 미니언이 들어가게 하거나 시야를 확보한 뒤 지난다. 묘목이 달려오면 아군 웨이브로 끌고 가지 않고 빈 공간으로 빠져 폭발 범위를 분리한다.

## 패시브가 준비된 막타를 압박한다

**마오카이는 준비된 기본 공격으로 체력을 회복한다.** 체력이 낮은 미니언 앞에 서서 회복 대상을 쉽게 주지 않으면 유지력이 줄어든다. 다만 회복 한 번을 막으려다 `Q`로 적 웨이브 안에 밀려나지 않도록 미니언 수와 위치를 먼저 본다.
...
- [마오카이에 대한 팁들](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=141364&vtype=pc)
- [시즌 8 마오카이 공략](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=141885&vtype=pc)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**나서스를 포탑 아래에 영원히 가두겠다고 계속 밀면 안전한 `Q` 막타와 정글의 갱 경로를 함께 준다.** 초반 웨이브를 포탑에 넣었다면 다음 웨이브가 내 쪽으로 돌아오게 두고, 긴 탑 라인 중앙에서 나서스가 막타를 먹으러 나오도록 만든다.

## `Q` 막타를 먹는 순간에만 정확히 때린다

**나서스가 모든 미니언을 먹는 동안 의미 없이 기술을 쓰기보다 `Q`로 막타를 치는 고정된 순간에 견제를 맞힌다.** 체력 우위를 만들었다면 대포 미니언 앞에서 압박하고, 적 웨이브가 더 많을 때는 스택 하나를 막겠다고 맞딜하지 않는다.
...
- [TOP 나서스에 대해](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=133482&vtype=pc)
- [TOP 나서스, 한 번 해보지 않을래?](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=110707)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**자크가 기술을 맞힌 뒤 떨어뜨린 조각을 안전하게 밟으면 회복과 `W` 재사용 감소를 함께 막는다.** 하지만 조각을 밟으러 적 웨이브 안으로 들어가면 자크가 원하는 긴 교환이 열린다. 내 발 가까이 떨어진 조각부터 지우고 먼 것은 포기한다.

## `Q`에 붙잡히면 미니언 곁에서 떨어진다

**`탄성 주먹(Q)` 첫 타격을 맞은 뒤 자크가 다른 대상을 공격하면 둘이 충돌한다.** 아군 미니언이나 소환물 옆에 그대로 서지 않고 반대 방향으로 움직인다. 자크가 두 번째 대상을 찾지 못하면 기술의 핵심 제어가 사라진다.
...
- [탑, 미드 자크 공략](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=146717)
- [자크장인 탑자크 공략](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=115999)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**탐 켄치에게 세 중첩이 쌓인 뒤 빠지려 하면 `Q` 기절이나 `R` 격리가 이어진다.** 체력바 주변 중첩을 보고 두 번째에서 거리를 만들기 시작한다. 세 번째 기본 공격을 피하려고 적 포탑 방향으로 도망가지 않고, 내 미니언 뒤로 돌아 `Q`까지 막는다.

## `Q`와 내 몸 사이에 미니언을 둔다

**`혀 채찍(Q)`은 첫 유닛에 막힌다.** 원거리 미니언 옆에서 각도를 유지하고, 탐 켄치가 옆으로 크게 돌아 선을 만들면 반대편으로 움직인다. `Q`가 미니언에 빠진 뒤에는 회복과 둔화가 모두 없으므로 짧게 압박한다.
...
- [탑 플래티넘 길라잡이](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=143266)
- [탐 켄치 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=126)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
...
**`오염된 뼈톱(Q)`은 처음 맞는 유닛에 막힌다.** 체력이 충분한 미니언 뒤에서 각도를 유지하고, 낮은 체력 미니언은 `E`에 밀려 투사체가 될 수 있으니 바로 뒤에 서지 않는다. `Q`가 미니언에 빠졌다면 둔화가 없는 시간에 막타나 짧은 견제를 챙긴다.

## 첫 군중 제어기는 처치보다 패시브 제거가 목적이다

**문도 박사는 처음 맞는 이동 불가 효과를 막고 화학 물질을 떨어뜨린다.** 가장 긴 핵심 기술부터 쓰지 말고 짧은 제어기로 패시브를 먼저 벗긴다. 떨어진 물질을 문도가 줍기 전에 안전하게 밟을 수 있다면 회복과 패시브 재사용 감소를 함께 막는다.
...
- [문도 박사 연구 결과 및 분석 - 탑](https://lol.inven.co.kr/dataninfo/champion/manualToolView.php?idx=112977&vtype=pc)
- [문도 박사 챔피언 정보](https://lol.inven.co.kr/dataninfo/champion/detail.php?code=13)', revision = 1, patch = '16.17.1', updated_at = '2026-09-13T06:00:00.000Z', updated_by = 'user-system'
